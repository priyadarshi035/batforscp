...
if %fileSize% GTR %temp1% (
   ...
   goto :loop 
    ...
    echo %NewestFile%
    FOR /F "delims=|" %%I IN ('DIR "%DatabaseBackupPath%\*.001" /B /O:D') DO SET  NewestFile=%%I
    curl -k -i -H "X-JFrog-Art-Api: " -X PUT  "https://gbsartifactory.in.edst.ibm.com/artifactory/ScopePlus/%1-db_backup_Maverick/%NewestFile%" -T %NewestFile%
    echo Build_Number %1
...
curl -X POST http://74:8080/view/Maverick_DBDeploy/job/Maverick_ArtifactoryNotify/buildWithParameters?Error_Message=Artifactory_Backup_error --user admin:
:exitloop
